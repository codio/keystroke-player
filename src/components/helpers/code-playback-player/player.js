...
const MONACO_FONT = '"Source Code Pro", Monaco, Menlo, Consolas, "Courier New", monospace'
...
const CodePlaybackEditor = React.forwardRef((props, ref) => {
  ...
  useImperativeHandle(ref, () => ({
    deltaDecorations: (data) => {
      if (monacoRef.current) {
        const decorations = _.map(data, item => {
          const range = monaco.Range.fromPositions(item.range.start, item.range.end)
    ...
    smartScrollToPosition(position) {
      if (monacoRef.current) {
        const {startLineNumber, endLineNumber} = monacoRef.current.editor.getVisibleRanges()[0]
        if (position.lineNumber > startLineNumber && position.lineNumber < endLineNumber - MIN_VISIBLE_LINES) {
