...
class ChangePlayer {
  ...
  _loadFileState(offset) {
    return CodePlaybackStore.loadFileState(this.projectId, this.filename, offset).then(fileState => {
  ...
  _loadAllStates() {
    const chunksP = []
    const chunksLength = Math.floor(this.fileTimeline.endPosition / this.fileTimeline.step)
  ...
  init(loadAll) {
    return CodePlaybackStore.loadFileInitialState(this.projectId, this.filename).then(state => {
      ...
      this.loadingFramePromise = loadAll ? this._loadAllStates() : this._loadFileState(0)
      return this.loadingFramePromise.then(() => {
  ...
  getDataForPosition(position) {
    return new Promise((resolve) => {
      position = _.isNumber(position) ? position : this.fileTimeline.currentPosition
      ...
      if (_.isString(current)) {
        ...
        const {content, decorators, positionInfo} = this._getContentDetails(current, prev)
        resolve(new ChangePlayer.PositionData({
          content,
          decorators,
          positionInfo,
    ...
    if (current && _.isString(current.content)) {
      ...
      _.each(diff, (diffItem) => {
        ...
        if (op === -1 || op === 1) {
          ...
          decorators.push(new ChangePlayer.Decorator({
            range,
            options: new ChangePlayer.DecoratorOptions({
              inlineClassName: op === -1 ? 'codePlaybackRemove' : 'codePlaybackInsert'
            })
          }))
        }
        ...
        if (!detectedScrollInfo) {
          detectedScrollInfo = new ChangePlayer.Position({ lineNumber, column })
        }
      })
    } else {
      content = newContent
