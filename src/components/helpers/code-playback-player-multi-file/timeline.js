...
const groups = [
  ...
  {id: GROUP_COPY_PASTE, content: 'Pastes', className: `timelineGroup${GROUP_COPY_PASTE}`}
]
...
const generateTimelineData = (positions, cases) => {
  ...
  positions.forEach((item, index) => {
    ...
    if (!nextItem || isSessionExpired || isFileChanged) {
      ...
      if (nextItem && (nextItem.date.getTime() - endRange.getTime()) > MINUTE_MS) {
        hiddenDates.push({
          start: endRange.getTime() + MINUTE_MS,
          end: nextItem.date.getTime() - 1
        })
      }
      items.push(getTimelineItem(null, GROUP_FILE, item.fileName.split('/').pop(), startRange, endRange, item.fileName))
      startRange = null
    }
  })
  if (cases) {
...
const Timeline = React.forwardRef((props, ref) => {
  ...
  useEffect(() => {
    ...
    timelineRef.current = new VisTimeline(elRef.current, items, groups, options)
